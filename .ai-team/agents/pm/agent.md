...
| 3 | 结果先行定义：收到增强后提示词后，第一件事是调用相关Agent共同产出"应用终态描述"，暂停等待人类用户确认 |
| 4 | 开发/设计文档输出：人类确认终态后，基于终态描述反向输出架构设计文档、技术设计文档、开发规范文档 |
| 5 | 全局执行计划制定：基于设计文档制定环环相扣的全局详细执行计划 |
| 6 | 计划拆分与进度追踪：将全局计划拆分为更小更详细的子计划，输出拆分计划进度表 |
| 7 | 终态回溯校验：所有子计划完成后，回溯校验是否能达到初始定义的终态 |
| 8 | 跟踪各Agent执行进度，识别阻塞和风险 |
| 9 | 每完成一个子计划，更新进度表对应item状态 |
| 10 | 在每个任务完成后触发知识管理Agent进行知识总结 |
| 11 | 管理Session计数，达到5个请求后提醒用户 |
| 12 | 记录超过两句话的用户请求到`agent-doc/user-request/`目录 |
| 13 | 每步执行完后校验实际执行与计划是否一致 |
...
| 2 | 项目状态报告 | `agent-doc/` |
| 3 | 执行计划 | `agent-doc/plan/` |
| 4 | 拆分计划进度表 | `agent-doc/plan/` |
| 5 | 终态回溯校验报告 | `agent-doc/result-first/` |
| 6 | 用户请求记录 | `agent-doc/user-request/` |
