...
| 全局执行计划概览 | 轻量概览文档 | 仅含任务清单和依赖关系（无详细步骤） |
| 独立子计划文件 | N个独立文件 | 每个任务一个独立 `agent-doc/plan/[日期]/SP-XX-xxx.md`，从蓝图直引最终结果 |
| 进度追踪总表 | 轻量摘要 | `agent-doc/plan/[日期]/progress.md`，仅状态追踪 |
...
项目根目录/
└── agent-doc/                   ← 所有产出物根目录
    ├── agent-doc/plan/                      ← 执行计划（全局概览 + 独立子计划文件）
...
| **结果先行** | **global-execution-planning** | **制定轻量全局执行计划概览（仅任务+依赖）** |
| **结果先行** | **plan-breakdown-tracking** | **拆分为独立子计划文件，每文件从蓝图直引最终结果，Agent无需参考其他文档** |
